...
#if defined(NDEBUG)
# error "BFFcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x66eaa2a4047364eacabae49627297a8bcce7c9aebeadb5bc02725c9c70684b2d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BFFcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BFFcoin Signed Message:\n";
...
{
    // BFFcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 5)  
    {
        nSubsidy = 1750000000 * COIN;
    }	

    // Subsidy is cut in half every 25000000 blocks, which will occur approximately every 120 years
    nSubsidy >>= (nHeight / 25000000); // BFFcoin: 25 mil blocks in ~120 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 25 * 60; // BFFcoin: 25 minutes
static const int64 nTargetSpacing = 2.5 * 60; // BFFcoin: 150 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BFFcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BFFcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x66eaa2a4047364eacabae49627297a8bcce7c9aebeadb5bc02725c9c70684b2d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BFFcoin Starts sending coin to your BFF";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("bf78z817u90i89ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1521891556;
        ...
        assert(block.hashMerkleRoot == uint256("0x9dbfefff2b4a975635a603e0dcf8f89a6b028c1b445344c589104f620845cc28"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // BFFcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BFFcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BFFcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BFFcoinMiner(CWallet *pwallet)
{
    printf("BFFcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bffcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BFFcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BFFcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BFFcoinMiner, pwallet));
}
